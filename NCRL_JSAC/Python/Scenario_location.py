...
sys.path.insert(1, '../')
from environment_fixed import UAVGridEnvTime as env
from environment_fixed import UAVGridEnvTime as env_eval
...
#    E = np.random.uniform(E_min,E_max,num_IoTD)
    E = np.array([31552.0861069276, 36155.4900574205, 32098.664907186, 33486.7819590322,48770.4603858191,30933.863584163,47335.1842219002,30985.785444386,30185.1206022113,38111.1267055511])
...
#    xs = np.random.uniform(0,x_max,1)
#    ys = np.random.uniform(0,y_max,1)
    xs = np.array([0])
    ys = np.array([500])
#    xi = np.random.uniform(0,x_max,num_IoTD + 1)
#    yi = np.random.uniform(0,y_max,num_IoTD + 1)
#    xi = np.random.uniform(0,x_max,num_IoTD)
#    yi = np.random.uniform(0,y_max,num_IoTD)
    xi = np.random.uniform(0,x_max * 0.01,5)
    yi = np.random.uniform(0,y_max * 0.01,5)
    xi = np.append(xi,np.random.uniform(x_max*0.99,x_max,5))
    yi = np.append(yi,np.random.uniform(y_max*0.99,y_max,5))
    xi = np.append(xi,500)
    yi = np.append(yi,500)
    
#    w = np.random.uniform(0,1,num_IoTD)
#    w = w/np.sum(w)
    w = np.array([0.0189038134564852, 0.194257712941318, 0.136039714331600, 0.00970733987021267, 0.0431297840066249, 0.147028336844155, 0.0910918664819089, 0.169341934248843, 0.0278833773041252, 0.162616120514727])
    
    dir_name = './results'
    if not os.path.exists(dir_name):
        os.mkdir(dir_name)

    log_file_name = dir_name + '/location_num_IoTD_{}_congested.txt'.format(num_IoTD) 
    
    
    file_name = dir_name + '/location_num_IoTD_{}_congested.mat'.format(num_IoTD) 
    train_py_env = env(num_IoTD, E,E_max,E_min,T,V_max,height,xs,ys,xi,yi,w)
    eval_py_env = env_eval(num_IoTD, E,E_max,E_min,T,V_max,height,xs,ys,xi,yi,w)
    ...
    with open(log_file_name,"a") as log_file:
          log_file.write('E = {0} \n xi = {1} \n yi = {2} \n xs = {3} \n ys = {4} \n T = {5} \n v = {6} \n height = {7} \n w = {8}'.format(E, xi, yi, xs, ys, T, V_max, height, w))
...
#        with open(log_file_name,"a") as log_file:
#          log_file.write('step = {0}: Average loss = {1}'.format(step, LOSS))
