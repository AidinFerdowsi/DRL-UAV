...
class UAVGridEnvTime(py_environment.PyEnvironment):
  ...
  def __init__(self, num_IoTD, E,E_max,E_min,T,V_max,height,xs,ys,xi,yi,w):
      ...
      self.T = T # the time limit
      self.V_max = V_max # speed maximum 
      self.height = height # minimum height at which the UAV is flying (nodes are at 0)
      self.xs = xs
      self.ys = ys
      self.xi = xi
      self.yi = yi
      ...
      self._observation_spec = array_spec.BoundedArraySpec(
              shape=(1,self.num_IoTD  + 1), dtype=np.float32, 
              minimum =  [-1] * (self.num_IoTD  + 1), maximum =  [1] * (self.num_IoTD  + 1),
              name='observation')
      self._state = [0] * (self.num_IoTD  + 1) # state (t,E,Loc,T,vmax,h,lambda)
      self._state[0] = -1
...
#      h = matlab.double(h.tolist(), size=(1, 1))
      h = matlab.double([self.height], size=(1, 1))
